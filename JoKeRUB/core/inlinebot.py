...
def main_menu():
    text = f"**- [سـورس كاتيا](https://t.me/Source_Katya)\
        \n المسـاعد\
        \n\
        \n◽ كاتيا لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس كاتيا 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس كاتيا 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس كاتيا 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Source_Katya"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/miika3",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت كاتيا 🧸♥](https://t.me/Source_Katya)**\
            \n\
            \n❤ كاتيا هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/miika3)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
