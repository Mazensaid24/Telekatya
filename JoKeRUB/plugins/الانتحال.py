...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @Source_Katya"
)
...
async def _(event):
    ...
    if replied_user.id == 5951674553:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 6816180621:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
    if replied_user.id == 6748769780:
        return await edit_delete(event, "**لا تحاول تنتحل المطورين ادبسز!**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @Source_Katya")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
