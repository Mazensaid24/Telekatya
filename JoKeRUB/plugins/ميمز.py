#all write Codes By Team Aljoker @Source_Katya
#By Hussein @Ve_G4
import asyncio
import random
import json
...
async def _(event):
    ...
    async with bot.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=2123712517)
            )
            await conv.send_message("/start")
...
async def aljoker313(joker313):
  ...
  await joker313.client.send_file(joker313.chat_id,url,caption="᯽︙ BY : @Source_Katya 🎀",parse_mode="html")
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @Source_Katya 🎀",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="قران$"))
async def jepvois(vois):
  rl = random.randint(2,101)
  url = f"https://t.me/QuraanJep/{rl}"
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @Source_Katya 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="᯽︙ THEME BY : @Source_Katya 🎊",parse_mode="html")
  await theme.delete()
...
async def Hussein(event):
    ...
    Ve_G4 = event.pattern_match.group(2)
    aljoker_links[Ve_G4] = url
    await event.edit(f"**᯽︙ تم اضافة البصمة {Ve_G4} بنجاح ✓ **")
...
async def Hussein(event):
    Ve_G4 = event.pattern_match.group(1)
    Joker = await reply_id(event)
    if Ve_G4 in aljoker_links:
        url = aljoker_links[Ve_G4]
...
async def delete_aljoker(event):
    Ve_G4 = event.pattern_match.group(1)
    if Ve_G4 in aljoker_links:
        del aljoker_links[Ve_G4]
        with open('aljoker_links.json', 'w') as file:
            json.dump(aljoker_links, file)
        await event.edit(f"**᯽︙ تم حذف البصمة '{Ve_G4}' بنجاح ✓**")
    else:
        await event.edit(f"**᯽︙ هذه البصمة '{Ve_G4}' غير موجودة في القائمة**")
    joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
    joker = Get(joker)
    try:
        await event.client(joker)
...
async def list_aljoker(event):
    if aljoker_links:
        ...
        for Ve_G4, url in aljoker_links.items():
            message += f"- البصمة : .`{Ve_G4}`\n"
    else:
